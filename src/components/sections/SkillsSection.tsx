      ...
      <div className="mx-auto max-w-7xl px-4 md:px-6">
        ...
        <div className="mt-12 grid gap-8 lg:grid-cols-2">
          <div className="skill-card relative mx-auto aspect-square w-full max-w-lg rounded-3xl p-4">
            ...
            <div className="absolute left-1/2 top-1/2 h-16 w-16 -translate-x-1/2 -translate-y-1/2 rounded-full bg-gradient-to-br from-[var(--text)] to-[var(--ember)] opacity-80 blur-sm" />
            <div className="absolute left-1/2 top-1/2 flex h-12 w-12 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-[var(--surface)] display text-[10px] text-[var(--text)] shadow-lg">
              CORE
            </div>
            {cms.skills.map((s, i) => {
              ...
              return (
                <motion.button
                  ...
                  className={cn(
                    ...
                    active === s.id
                      ? "h-16 w-16 border-[var(--gold)] bg-[var(--surface-2)] text-[var(--text)] shadow-[0_0_30px_rgba(var(--accent-rgb),0.45)]"
                      : "h-12 w-12 border-[color:rgba(var(--accent-rgb),0.4)] bg-[color-mix(in_srgb,var(--surface)_95%,transparent)] text-[color-mix(in_srgb,var(--text)_90%,transparent)] hover:scale-110",
                  )}
                  style={{ left: `${x}%`, top: `${y}%` }}
